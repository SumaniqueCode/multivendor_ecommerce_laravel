      ...
      <div class="container mx-auto relative flex items-center p-2 text-lg">
        ...
        <div class="hidden lg:flex space-x-4 ms-auto me-10">
          <a href="/cart" id="cart" class="text-white mx-2 p-1 rounded">
            <span class="bg-red-500 text-sm font-bold px-1 border border-1 border-white rounded-full absolute mini_cart_icon_number">{{$carts->sum('quantity')}}</span>
          ...
          <div id="cartdesc" class="cartdesc absolute right-16 top-12 border-4 border-gray-500 rounded-xl bg-white w-1/3">
            ...
            <table class="table table-dark w-full">
              <tbody class="">
                ...
                <tr>
                  <td rowspan="2" class="">
                    <img class="rounded my-auto h-20 w-24 ms-2" src="{{ asset(json_decode($cart->productVariation['image'])[0]) }}" alt="">
                  </td>
                  <td class="w-28">{{$cart->productVariation->product['product_name']}}</td>
                  <td class="flex gap-1 w-48">
                    <input type="hidden" id="miniCartData" name="cart_id" value="{{ json_encode(['cart' => $cart, 'product' => $cart->product]) }}" class="border rounded">
                    <label for="" class="my-auto mx-1">Quantity: </label>
                      <input class="w-36 pl-2 border rounded-xl" type="number" id="miniCartQuantity" name="quantity" value="{{$cart->quantity}}">
                  </td>
                </tr>
                <tr class="border-b-2 border-gray-500">
                  <td colspan="">
                    Rs. {{ $cart->productVariation['price'] * $cart->quantity }}
                  </td>
                  <td class="text-right">
                    <a class="me-2 my-auto" href="/delete-cart/{{$cart->id}}"><button class="px-5 py-1 bg-red-500 text-white font-semibold rounded-lg">Remove Cart</button></a>
                  </td>
                </tr>
