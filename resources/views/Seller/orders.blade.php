                ...
                <tbody>
                    @foreach ($orders as $order)
                        <tr class="bg-gray-100 border-b border-gray-200 text-gray-900 hover:bg-blue-300">
                            ...
                            <td scope="row" class="px-6 text-base font-medium whitespace-nowrap dark:text-blue-100">
                                <p class="orderStatus px-3 py-1 rounded text-white {{ $order->status == 'Pending' ? 'bg-orange-500' : ($order->status == 'Processing' ? 'bg-yellow-500' : ($order->status == 'Shipped' ? 'bg-blue-600' : ($order->status == 'Delivered' ? 'bg-green-600' : 'bg-red-600'))) }}">
                                    {{ $order->status }} </p>
                            </td>
                            <td scope="row" class="px-6 text-base font-medium whitespace-nowrap dark:text-blue-100">
                            ...
                            <td scope="row" class="flex px-6 text-base font-medium whitespace-nowrap dark:text-blue-100">
                                ...
                                <a href="/product/get-product-details/{{ $order->productVariation->product['id'] }}">
